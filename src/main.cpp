...
void setup() {
    ...
    if (!frontLidar.initialize(0x30)) {
        Serial.println("ERROR: Front LIDAR failed to initialize!");
        while(1) delay(100);
    }
    ...
    if (!leftLidar.initialize(0x31)) {
        Serial.println("ERROR: Left LIDAR failed to initialize!");
        while(1) delay(100);
    }
    ...
    if (!rightLidar.initialize(0x32)) {
        Serial.println("ERROR: Right LIDAR failed to initialize!");
        while(1) delay(100);
    }
    ...
    //------------------------------------------------------------
    Serial.println("Initializing maze navigator...");
...
//------------------------------------------------------------
void loop() {
    // Exploration complete, just idle
    delay(10000);
}
